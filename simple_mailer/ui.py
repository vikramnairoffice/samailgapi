...
from simple_mailer.orchestrator import ui_shell as _orchestrator_ui_shell
...
_UI_SHELL_FLAG = "SIMPLE_MAILER_UI_SHELL"
_UI_SHELL_ON_VALUES = {"1", "true", "yes", "on"}


def _is_ui_shell_enabled(env=None):
    source = env or os.environ
    raw_value = source.get(_UI_SHELL_FLAG, "")
    if not raw_value:
        return False
    return raw_value.strip().lower() in _UI_SHELL_ON_VALUES
...
def gradio_ui():
    if _is_ui_shell_enabled():
        return _orchestrator_ui_shell.build_ui()
