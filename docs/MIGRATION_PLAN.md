...
- Migrate `email_manual` first, then `email_automatic` (Bulk HTML), followed by `drive_share` and `multi_mode`, registering each mode with the orchestrator.
- Legacy UI shell is now retired; the orchestrator shell stays the default entry point once parity completes (Gate 6).
